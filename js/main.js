$(function(){
	$.get("data/music.json",{},function(result){
...
function musicLrc(result){
	...
	lrcs.forEach(function(item,i){
		...
		//第二个 . 表示任意字符
		// * 表示出现0 次或多次 （任意次数）
		var time = item.substring(1,lastIndexOf - 1).replace(/\..*/g,"");
...
$(function(){
	...
	$(audio).on("play",function(){
		...
		Interval = setInterval(function(){
			...
			var top=$('#box').find('.lrc').position().top;
			
			$active.addClass("active");
			$active.siblings(".active").removeClass("active");
			var y=$('.lrc').children('.active').offset().top;
			//console.log($('.lrc').children('.active').offset().top)
			if(y>200){
				top=top - 24 +"px";
				$('#box').find('.lrc').css("top",top);
			}
